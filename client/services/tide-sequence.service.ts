...
export class TideSequenceService {
    ...
    private createCycle(cycleData: { startTime?: string; points: TideData[] }): TideCycle | null {
        ...
        const lowestPoint = cycleData.points.reduce((min, point) =>
            point.height < min.height ? point : min
            , cycleData.points[0]);

        const highestPoint = cycleData.points.reduce((max, point) =>
            point.height > max.height ? point : max
            , cycleData.points[0]);
